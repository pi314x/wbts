...
async function chainList(short = null) {
  try {
    ...
    try { switchChainId = chaindata['chainId'] } catch (e) { switchChainId = -1 }
    try { switchExplorer = chaindata['explorers'][0]['url'] } catch (e) { switchExplorer = "" }
    try { switchContract = contracts['contracts'][networkValue] } catch (e) { switchContract = "" }
    try { networkTxt = chaindata['title'] } catch (e) { networkTxt = "" }
